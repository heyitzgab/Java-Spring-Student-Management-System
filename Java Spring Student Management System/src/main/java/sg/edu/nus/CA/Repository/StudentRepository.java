...
public interface StudentRepository extends JpaRepository<Student, Integer> {
	...
	public Student findxByid(int id);

	@Transactional
	@Modifying
	@Query("UPDATE Student s SET s.mobile = :mobileNO,s.EmailID = :mailID,s.address = :Address WHERE s.id = :sid")
	void updateDetails(@Param("sid") int id, @Param("mobileNO") int mobileNO, @Param("mailID") String mailID,
	...
	void updateStudent(@Param("sid") int id, @Param("sname") String name, @Param("gender") String gender,
	...
	void updateScore(@Param("sid") int id, @Param("sgpa") double gpa);

	@Transactional
	@Modifying
	@Query("UPDATE Student s SET s.sessionid = :SessionID WHERE s.id = :sid")
	void updateSessionId(@Param("sid") int id, @Param("SessionID") String SessionID);

	@Query("select s.id from Student s where s.sessionid = :SessionID")
	public int GetidbySessionID(@Param("SessionID") String SessionID);
	...
	void insertStudentCourse(@Param("stdID") Integer stdID, @Param("courseid") Integer courseid,
